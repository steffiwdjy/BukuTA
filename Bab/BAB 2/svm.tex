\section{{\it Support Vector Machine} (SVM)}\label{sec:svm}

{\it Support Vector Machine} (SVM) merupakan algoritma klasifikasi yang bertugas mencari batas pemisah ({\it hyperplane}) paling optimal untuk membedakan dua kelompok data. SVM bekerja dengan mencari garis (dalam dua dimensi) atau bidang (dalam dimensi lebih tinggi) yang memaksimalkan jarak antara dua kelas data yang berbeda \cite{cortes1995support}. Dalam praktiknya, SVM efektif digunakan untuk klasifikasi data linier maupun nonlinier melalui fungsi kernel, seperti \textit{Radial Basis Function} (RBF), polinomial, dan sigmoid \cite{hastie2009elements}.
    ...
    Sebagai ilustrasi, simulasi penentuan batas keputusan berikut diadaptasi dari formulasi \textit{optimal separating hyperplane} yang dijabarkan dalam literatur \textit{The Elements of Statistical Learning} \cite{hastie2009elements}, yang kemudian diaplikasikan pada konteks data penyewa. Misalkan terdapat dua fitur penyewa yang dinilai: lama menginap ($x_1$) dan waktu \textit{check-in} yang sudah diubah menjadi angka ($x_2$, misalnya pagi = 0, sore = 1, malam = 2). Setelah model dilatih dan bobot kepentingannya ($w$) serta nilai biasnya ($b$) disesuaikan oleh algoritma, sistem menemukan rumus keputusan batas sebagai berikut.
    $$
    f(x) = 2x_1 + 3x_2 - 5
    $$
    ...
    Sesuai dengan teori dari Christopher M. Bishop  \cite{bishop2006pattern} dalam \textit{Pattern Recognition and Machine Learning}, yang menjelaskan bahwa kernel RBF memungkinkan model proyeksi pemetaan ke dalam ruang dimensi tak terhingga. Fleksibilitas ini secara dramatis meningkatkan daya jangkau algoritma, sehingga mampu membungkus dan mengisolasi wilayah kelas minoritas tanpa kesulitan yang berarti meski posisinya berpencar-pencar. Kemampuan RBF dalam membungkus data berbentuk sebaran kurva geometris melingkar, menjadikannya senjata mumpuni bagi SVM saat memilah distribusi sebaran data rentan (\textit{non-linear}) \cite{burges1998tutorial}.
    
    Algoritma SVM dikenal sangat tangguh untuk memproses data yang memiliki banyak kolom fitur. Selain itu, SVM juga cukup kebal terhadap kecenderungan menghafal data secara berlebihan ({\it overfitting}), terutama pada dataset yang minim gangguan ({\it noise}). Karena sifatnya yang mendefinisikan batas keputusan eksplisit, SVM banyak digunakan dalam bidang deteksi intrusi, identifikasi penipuan, dan klasifikasi risiko, termasuk dalam pengawasan properti berbasis data \cite{scholkopf2002learning}.
